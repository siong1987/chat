...
module Juggernaut
  class Client
    ...
    def send_message(msg, channels = nil, without_sign = false)
      store_message(msg, channels) if options[:store_messages]
      send_message_to_connections(msg, channels, without_sign)
    end
    ...
    def send_message_to_connections(msg, channels, without_sign = false)
      @connections.each do |em|
        em.broadcast(msg, without_sign) if !channels or channels.empty? or em.has_channels?(channels)
      end
    end
