...
module Juggernaut
  class Server < EventMachine::Connection
    ...
    def broadcast(bdy, without_sign = false)
      logger.debug "Without sign: #{without_sign}"
      if without_sign
        msg = bdy + ' '
      else
    ...
    protected
      ...
      def broadcast_command
        ...
        case @request[:type].to_sym
          ...
          when :to_channels_without_signature
            broadcast_to_channels(@request[:body], @request[:channels], true)
          when :to_clients
    ...
    private
      ...
      def broadcast_to_channels(msg, channels = [], without_sign = false)
        Juggernaut::Client.find_all.each {|client| client.send_message(msg, channels, without_sign) }
      end
